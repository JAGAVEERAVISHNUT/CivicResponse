...
import { L2IssueActions } from "@/components/l2-issue-actions";
import { IssueImageGallery } from "@/components/issue-image-gallery"; // Added image gallery import
...
export default async function L2IssueDetailPage({ params }: IssueDetailPageProps) {
  ...
  if (!profile || profile.role !== 'l2_officer') {
    redirect("/auth/login");
  }
  ...
  if (issue.assigned_l2_id !== user.id) {
    redirect("/l2-dashboard");
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8 max-w-5xl">
        <Button variant="ghost" asChild className="mb-4">
          <Link href="/l2-dashboard">← Back to Dashboard</Link>
        </Button>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            {/* Actions Card */}
            <L2IssueActions issue={issueWithProfiles} />
            ...
            <Card>
              ...
              <CardContent className="space-y-4 text-sm">
                ...
                {issueWithProfiles.assigned_l2_at && (
                  <div className="flex items-center gap-2">
                    <Calendar className="h-4 w-4 text-muted-foreground" />
                    <div>
                      <p className="text-muted-foreground">Assigned to Me</p>
                      <p className="font-medium">{format(new Date(issueWithProfiles.assigned_l2_at), 'MMM dd, yyyy HH:mm')}</p>
                    </div>
                  </div>
                )}
              ...
              <CardContent className="space-y-4 text-sm">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{issueWithProfiles.reporter?.email}</p>
                    {issueWithProfiles.reporter?.phone && (
                      <p className="text-xs text-muted-foreground">{issueWithProfiles.reporter.phone}</p>
                    )}
                  </div>
                </div>
